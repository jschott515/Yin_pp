...
void Yin::load_buffer()
{
	// temp hacky check if the file's done processing
	if (audioBuffer_idx + w_step >= 240000)
	{
		done = true;
		return;
	}
	// load data to buffer as int
	memcpy(in_buff[head_idx], &audioBuffer[audioBuffer_idx], w_step * sizeof(int));
