...
public:
	...
	void compute_yin();        // Step the input buffer and evaluate a new pitch/harmonic rate
	float get_pitch();         // returns most recent calculated pitch 
	float get_harmonic_rate(); // returns most recent calculated harmonic rate

	// Temp flag to signal when the audio file is done being processed
	bool done = false;
...
private:
	...
	// Class Methods --------------------------------------------------
	void load_buffer();              // Load step size of samples into input buffer as floats
